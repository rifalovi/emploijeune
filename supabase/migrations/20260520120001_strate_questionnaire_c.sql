-- =============================================================================
-- Plateforme OIF Emploi Jeunes — Migration 027 : questionnaire C dans strate
-- -----------------------------------------------------------------------------
-- Le questionnaire C (Intermédiation vers l'emploi) cible les bénéficiaires
-- exactement comme le questionnaire A. Les 3 fonctions strate traitent donc
-- 'C' identiquement à 'A' (table beneficiaires, colonne annees, filtres sexe
-- et tranche_age disponibles).
--
-- Modifications :
--   1. lister_strate      → IF p_questionnaire = 'A' → IN ('A', 'C')
--   2. compter_strate     → IF p_questionnaire = 'A' → IN ('A', 'C')
--   3. lister_strate_ids  → IF p_questionnaire = 'A' → IN ('A', 'C')
--
-- Compatibilité ascendante :
--   Toutes les campagnes A et B existantes continuent de fonctionner sans
--   changement de comportement. Seul le nouveau cas 'C' est ajouté.
-- =============================================================================


-- ─────────────────────────────────────────────────────────────────────────────
-- 1. lister_strate — étendre la branche bénéficiaire à C
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Questionnaires A et C → bénéficiaires
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. compter_strate — étendre la branche bénéficiaire à C
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. lister_strate_ids — étendre la branche bénéficiaire à C
-- ─────────────────────────────────────────────────────────────────────────────
...
COMMENT ON FUNCTION public.lister_strate IS
  'Retourne les cibles paginées. Questionnaires A et C → bénéficiaires ; B → structures. Migration 027.';

COMMENT ON FUNCTION public.compter_strate IS
  'Compte les cibles éligibles. Questionnaires A et C → bénéficiaires ; B → structures. Migration 027.';

COMMENT ON FUNCTION public.lister_strate_ids IS
  'Retourne les UUID des cibles éligibles. Questionnaires A et C → bénéficiaires ; B → structures. Migration 027.';
