...
export type StructurePaginationProps = {
  page: number;
  pageSize: number;
  total: number;
  totalPages: number;
...
export function StructurePagination({
  page,
  pageSize,
  total,
  totalPages,
}: StructurePaginationProps) {
  const { setParams } = useStructureFilters();
  ...
  return (
    <div className="flex flex-col gap-3 pt-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-muted-foreground text-sm">
        {total > 0 && (
          <>
            Affichage {debut}-{fin} sur <strong>{total.toLocaleString('fr-FR')}</strong> structure
