...
export type BeneficiairePaginationProps = {
  page: number;
  pageSize: number;
  total: number;
  totalPages: number;
...
export function BeneficiairePagination({
  page,
  pageSize,
  total,
  totalPages,
}: BeneficiairePaginationProps) {
  const { setParams } = useBeneficiaireFilters();
  ...
  return (
    <div className="flex flex-col gap-3 pt-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-muted-foreground text-sm">
        {total > 0 && (
          <>
            Affichage {debut}-{fin} sur <strong>{total.toLocaleString('fr-FR')}</strong>{' '}
            bénéficiaire{total > 1 ? 's' : ''} · Page {page} sur {totalPages}
          </>
        )}
      </p>
