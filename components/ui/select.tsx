...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        // `w-full` (au lieu de `w-fit`) CRITIQUE (hotfix 4g) : sans contrainte,
        // le trigger s'élargit pour accommoder les libellés longs et déborde
        // sur les cellules voisines de la grille du formulaire (ex.
        // « PROJ_A03 — Initiative francophone pour la formation à distance
        // des maîtres (IFADEM) » empiétant sur le champ Pays). Avec `w-full`
        // + `min-w-0`, la cellule de grille impose la largeur et
        // `line-clamp-1` sur la valeur applique l'ellipsis.
        "border-input focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-placeholder:text-muted-foreground dark:bg-input/30 dark:hover:bg-input/50 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 flex w-full min-w-0 items-center justify-between gap-1.5 rounded-lg border bg-transparent py-2 pr-2 pl-2.5 text-sm whitespace-nowrap transition-colors outline-none select-none focus-visible:ring-3 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:ring-3 data-[size=default]:h-8 data-[size=sm]:h-7 data-[size=sm]:rounded-[min(var(--radius-md),10px)] *:data-[slot=select-value]:block *:data-[slot=select-value]:min-w-0 *:data-[slot=select-value]:flex-1 *:data-[slot=select-value]:truncate [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
