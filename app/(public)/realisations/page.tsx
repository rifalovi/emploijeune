        ...
        <section className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {piliers.map((p) => {
            ...
            return (
              <Link
                key={p.code}
                href={`/realisations/${p.code.toLowerCase()}`}
                className="group block"
              >
                <Card
                  className="h-full overflow-hidden border-t-4 transition-all hover:-translate-y-1 hover:shadow-xl"
                  style={{ borderTopColor: p.couleur }}
                >
                  <CardContent className="flex h-full flex-col p-6">
                    <div className="flex items-center justify-between">
                      <span
                        className="inline-flex size-12 items-center justify-center rounded-xl text-xl font-extrabold text-white shadow"
                    ...
                    <h2 className="mt-4 text-lg font-bold" style={{ color: p.couleur }}>
                      {p.titre}
                    </h2>
                    ...
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {indicateurs.map((i) => (
                        <span
                          ...
                          style={{ borderColor: `${p.couleur}44`, color: p.couleur }}
                        >
                          {i.code}
                        </span>
                      ))}
